...
public sealed record RestoreNoShowTicketCommand(
    Guid PracticeId,
    Guid TicketId,
    string RowVersion,
...
internal sealed class RestoreNoShowTicketCommandValidator
    : AbstractValidator<RestoreNoShowTicketCommand>
{
    public RestoreNoShowTicketCommandValidator()
    {
...
internal sealed class RestoreNoShowTicketCommandHandler(
    ...
    : ICommandHandler<RestoreNoShowTicketCommand, TicketDetailsResponse>
{
    public async Task<Result<TicketDetailsResponse>> Handle(
        RestoreNoShowTicketCommand request,
        CancellationToken cancellationToken)
    {
        ...
        var currentBusinessDate = TicketBusinessClock.CurrentBusinessDate(
            nowUtc, configuration.TimeZoneId);
        if (ticket.BusinessDate != currentBusinessDate)
        {
        ...
        await queueLock.AcquireIdempotencyAsync(
            actor.Value.ApplicationUserId, "RestoreNoShowTicket", key.Value, cancellationToken);
        ...
        var passedPatients = await queueReader.CountInProgressTransitionsAfterAsync(
            request.PracticeId,
            ticket.BusinessDate,
            ticket.NoShowOnUtc.Value,
            cancellationToken);
        var restored = ticket.RestoreNoShow(
            actor.Value.ApplicationUserId,
            passedPatients <= configuration.TicketNoShowReturnFastTrackLimit,
            nowUtc);
        if (restored.IsFailure)
        {
            return Result<TicketDetailsResponse>.Fail(restored.Errors);
        }
